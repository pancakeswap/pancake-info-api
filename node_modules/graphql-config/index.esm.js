import { __extends, __spreadArrays, __awaiter, __generator, __assign } from 'tslib';
...
import { loadTypedefs, loadTypedefsSync, loadDocuments, loadDocumentsSync, loadSchema, OPERATION_KINDS, loadSchemaSync } from '@graphql-toolkit/core';
...
function replaceEnv(content) {
    ...
    return content.replace(R, function (_, result) {
        ...
        if (value) {
            if (rest && rest.length) {
                value = __spreadArrays([value], rest).join(':');
            }
            value = value.trim();
            if (value.startsWith("\"")) {
...
var LoadersRegistry = /** @class */ (function () {
                ...
                switch (_b.label) {
                    case 0:
                        ...
                        if (out === 'GraphQLSchema' && !this._middlewares.length) {
                            return [2 /*return*/, loadSchema(pointer, options)];
                        }
                        _a = this.transformSchemaSources;
                        return [4 /*yield*/, loadTypedefs(pointer, __assign({ filterKinds: OPERATION_KINDS }, options))];
                    case 1:
    ...
    LoadersRegistry.prototype.loadSchemaSync = function (pointer, out) {
        ...
        var schemaDoc = this.transformSchemaSources(loadTypedefsSync(pointer, __assign({ filterKinds: OPERATION_KINDS }, options)));
        return this.castSchema(schemaDoc, out);
    };
    LoadersRegistry.prototype.createOptions = function (options) {
        return __assign({ loaders: this._loaders, cwd: this.cwd }, options);
    };
